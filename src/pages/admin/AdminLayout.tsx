...
const navItems = [
  ...
  { label: 'Folders', icon: FolderTree, href: '/admin/folders' },
  { label: 'Approvals', icon: CheckCircle, href: '/admin/approvals', badge: 3 },
  { label: 'Users', icon: Users, href: '/admin/users' },
  { label: 'Analytics', icon: BarChart3, href: '/admin/analytics' },
  { label: 'Recycle Bin', icon: Trash2, href: '/admin/trash' },
];
      ...
      <aside className="hidden w-64 flex-col border-r border-border bg-sidebar lg:flex">
        <div className="flex items-center gap-3 border-b border-sidebar-border p-4">
          ...
          <div className="min-w-0">
            <p className="text-sm font-bold text-sidebar-foreground truncate">Admin Panel</p>
        ...
        <nav className="flex-1 p-3 space-y-0.5">
          {navItems.map(item => {
            ...
            const allowed = user?.role === 'dos' || !['Users', 'Analytics', 'Recycle Bin'].includes(item.label);
        ...
        <header className="sticky top-0 z-40 flex items-center justify-between border-b border-border bg-card px-6 py-3 header-shadow">
          ...
          <div className="hidden lg:block">
            ...
            <p className="text-xs text-muted-foreground">
              {user?.role === 'dos' ? 'Director of Studies' : 'Teacher'} · {user?.email}
            </p>
          </div>
